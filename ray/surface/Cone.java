...
public class Cone extends Surface {
	...
	public void setTipz(double tipz) {
		this.tipz = tipz;
	}
	...
	/**
	 * The height of the cone. Truncation of the cone occurs at center.z -
	 * height/2 and center.z + height/2
	 */
	...
	public boolean intersect(IntersectionRecord outRecord, Ray rayIn) {
		...
		double tw = Double.POSITIVE_INFINITY; // t value for intersection with
												// cone wall
		double te = Double.POSITIVE_INFINITY; // t value for intersection with
												// end caps
		...
		if (discr >= 0.0) {
			...
			if (Math.abs(q.z - center.z) > height / 2) // outside truncated
														// range
				t1 = Double.POSITIVE_INFINITY;
			double t2 = (-B - sd) / (2 * A);
		...
		if (inters) {
			...
			if (tw < te) { // intersection with wall
				...
				outRecord.normal.z = R / H; // set z component according to
											// slope of cone
